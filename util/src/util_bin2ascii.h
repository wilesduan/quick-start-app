...
inline void b64_encode(const std::string &s, std::string& r)
{
...
inline int b64_decode(const std::string &s, std::string& r)
{
	typedef unsigned char u1;
	static const char lookup[] = ""
		"\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80" // 0x00
	...
	if (!s.size()) return -1;
	if (s.size() % 4){
		LOG_ERR("invalid base64 data size");
		return -2;
	}
