 ...
 * UTIL_MD5Context structure, pass it to MD5Init, call MD5Update as
 ...
 */
void
MD5Update(UTIL_MD5_CTX *ctx, const unsigned char *input, size_t len)
{
	size_t have, need;
 ...
 */
void
MD5Final(unsigned char digest[MD5_DIGEST_LENGTH], UTIL_MD5_CTX *ctx)
{
	uint8_t count[8];
	size_t padlen;
	int i;
...
{
	bzero(md5sum, MD5_DIGEST_LENGTH);
	UTIL_MD5_CTX ctx;

	MD5Init(&ctx);
	MD5Update(&ctx, (unsigned char*)key, key_len);
	MD5Final(md5sum, &ctx);
}
...
size_t get_index_from_key(const void* key, size_t key_len)
{
	...
	md5_sum(key, key_len, md5);
	return get_hash_from_md5(md5);
}

unsigned get_hash_from_md5(unsigned char* md5sum)
{
	return m_hash(md5sum, 16);

	/*
	unsigned hash = 0;
	unsigned char* p = (unsigned char*)(&hash);
	size_t step = 16/sizeof(unsigned);
