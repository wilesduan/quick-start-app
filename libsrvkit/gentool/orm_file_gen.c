...
static const char* auto_header = "//Generated by codegen, DO NOT EDIT(IF NEEDED, pls contact the author: wilesduan@gmail.com). \n\n"; 
...
static void gen_all_member_construct(FILE* fp, const char* indent, const std::string& name, const std::vector<t_field>& fields)
{
	if(!fields.size()){
		return;
	}
	...
	for(size_t i = 0; i < fields.size(); ++i){
		const t_field& field = fields[i];
		EN_MYSQL_FIELD_TYPE type = get_mysql_field_type(field.column_type.data());
		params.append(field_2_str[type].data_type).append(" ").append(field.column_name);
		assignment.append(indent).append("\tthis->").append(field.column_name).append(" = ").append(field.column_name).append(";\n");
		switch(type){
			case EN_MYSQL_FIELD_STR:
			case EN_MYSQL_FIELD_BINARY:
			case EN_MYSQL_FIELD_VAR_STR:
				{
					params.append(", size_t ").append(field.column_name).append("_len");
					assignment.append(indent).append("\tthis->").append(field.column_name).append("_len = ").append(field.column_name).append("_len;\n");
				}
				break;
			default:
				break;
	...
	fprintf(fp, "%s%s(%s)\n%s{\n%s%s}\n", indent, name.data(), params.data(), indent, assignment.data(), indent);
}
...
void gen_select_header(FILE* fp, const std::string& package, const t_mysql_query& query)
{
	...
	struct_name.append(package).append("_select_").append(query.tag).append("_condition_t");
	...
	gen_all_member_construct(fp, "\t", struct_name, query.conditions);
...
void gen_insert_header(FILE* fp, const std::string& package, const t_mysql_query& query)
{
	...
	std::string struct_name;
	struct_name.append(package).append("_insert_").append(query.tag).append("_columns_t");
	gen_all_member_construct(fp, "\t", struct_name, query.columns);
	...
	struct_name = "";
	struct_name.append(package).append("_insert_").append(query.tag).append("update_t");
	gen_all_member_construct(fp, "\t", struct_name, query.updates);
...
void gen_update_header(FILE* fp, const std::string& package, const t_mysql_query& query)
{
	...
	std::string struct_name;
	struct_name.append(package).append("_update_").append(query.tag).append("_column_t");
	gen_all_member_construct(fp, "\t", struct_name, query.columns);
	...
	struct_name = "";
	struct_name.append(package).append("_update_").append(query.tag).append("_condition_t");
	gen_all_member_construct(fp, "\t", struct_name, query.conditions);
